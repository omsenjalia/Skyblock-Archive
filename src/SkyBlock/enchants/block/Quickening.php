...
class Quickening extends BaseBlockBreakEnchant {
    ...
    public function onActivation(Player $player, BlockBreakEvent $ev, int $enchantmentlevel) : void {
        if ($ev->getBlock()->getBreakInfo()->getToolType() === BlockToolType::PICKAXE) {
            $effect = new EffectInstance(VanillaEffects::HASTE(), $enchantmentlevel * 2 * 20, $this->getLevel($enchantmentlevel), false);
