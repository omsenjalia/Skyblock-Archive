...
class TopMoney extends BaseCommand {
    public function __construct(Main $plugin) {
        parent::__construct($plugin, 'topmoney', 'Top money', '<page>', true, ['baltop']);
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) : void {
        ...
        $page = (int) $args[0];
        $array = Main::getInstance()->db->prepare("SELECT COUNT(*) AS COUNT FROM player ORDER BY money DESC;")->execute();
        ...
        $this->sendMessage($sender, TextFormat::YELLOW . "Top Money List -");
        ...
        $array = Main::getInstance()->db->prepare("SELECT player, money FROM player ORDER BY money DESC LIMIT $startNumber, 8;")->execute();
        ...
        while ($result) {
            ...
            if ($user === null) {
                $string .= TextFormat::WHITE . "$startNumber. $name => $" . number_format($money) . " \n";
            } else {
                $string .= TextFormat::WHITE . "$startNumber. $name => $" . number_format($user->getMoney()) . " \n";
            }
        }
