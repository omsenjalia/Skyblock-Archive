...
class TopBlocks extends BaseCommand {
    public function __construct(Main $plugin) {
        parent::__construct($plugin, 'topblocks', 'Top Blocks broken', '<page>', true, ['topblock', 'topblocksbroken', 'tbb']);
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) : void {
        ...
        $page = (int) $args[0];
        $array = Main::getInstance()->db->prepare("SELECT COUNT(*) AS COUNT FROM player ORDER BY blocks DESC;")->execute();
        ...
        $this->sendMessage($sender, TextFormat::YELLOW . "Top Blocks Broken List -");
        ...
        $array = Main::getInstance()->db->prepare("SELECT player, blocks FROM player ORDER BY blocks DESC LIMIT $startNumber, 8;")->execute();
        ...
        while ($result) {
            ...
            if ($user === null) {
                $string .= TextFormat::WHITE . "$startNumber. $name => $" . number_format($blocks) . " \n";
            } else {
                $string .= TextFormat::WHITE . "$startNumber. $name => $" . number_format($user->getBlocksBroken()) . " \n";
            }
        }
