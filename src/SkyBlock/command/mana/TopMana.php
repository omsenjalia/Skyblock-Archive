...
class TopMana extends BaseCommand {
    public function __construct(Main $plugin) {
        parent::__construct($plugin, 'topmana', 'Top mana', '<page>', true, ['manatop']);
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) : void {
        ...
        $page = (int) $args[0];
        $array = Main::getInstance()->db->prepare("SELECT COUNT(*) AS COUNT FROM player ORDER BY mana DESC;")->execute();
        ...
        $this->sendMessage($sender, TextFormat::YELLOW . "Top Mana List -");
        ...
        $array = Main::getInstance()->db->prepare("SELECT player, mana FROM player ORDER BY mana DESC LIMIT $startNumber, 8;")->execute();
        ...
        while ($result) {
            ...
            if ($user === null) {
                $string .= TextFormat::WHITE . "$startNumber. $name => $" . number_format($mana) . " \n";
            } else {
                $string .= TextFormat::WHITE . "$startNumber. $name => $" . number_format($user->getMana()) . " \n";
            }
        }
