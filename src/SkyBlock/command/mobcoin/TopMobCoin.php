...
class TopMobCoin extends BaseCommand {

    /**
     * TopMobCoin constructor.
     *
     * @param Main $plugin
     */
    public function __construct(Main $plugin) {
        parent::__construct($plugin, 'topmobcoin', 'Top mob coin', '<page>', true, ['topmc']);
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) : void {
        ...
        $page = (int) $args[0];
        $array = Main::getInstance()->db->prepare("SELECT COUNT(*) AS COUNT FROM player ORDER BY mobcoin DESC;")->execute();
        ...
        $this->sendMessage($sender, TextFormat::YELLOW . "Top Mobcoin List -");
        ...
        $array = Main::getInstance()->db->prepare("SELECT player, mobcoin FROM player ORDER BY mobcoin DESC LIMIT $startNumber, 8;")->execute();
        $result = $array->fetchArray(SQLITE3_ASSOC);
        while ($result) {
            $mobcoin = $result["mobcoin"];
            $name = $result["player"];
            $startNumber++;
            $user = Main::getInstance()->getUserManager()->getOnlineUser($name);
            if ($user === null) {
                $string .= TextFormat::WHITE . "$startNumber. $name => $" . number_format($mobcoin) . " \n";
            } else {
                $string .= TextFormat::WHITE . "$startNumber. $name => $" . number_format($user->getMobCoin()) . " \n";
            }
        }
