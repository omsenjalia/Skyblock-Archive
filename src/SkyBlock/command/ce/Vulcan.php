...
class Vulcan extends BaseCommand {

    /**
     * @param Main $plugin
     */
    public function __construct(Main $plugin) {
        parent::__construct($plugin, 'vulcan', 'Remove CE from tool and get it as CE book', '<ce name>');
    }
        ...
        foreach ($items as $slot => $item) {
            if ($item->getTypeId() === ItemTypeIds::PRISMARINE_SHARD) {
                if ($item->hasCustomName()) {
                    ...
                    $type = $spaces[1];
                    $type2 = $spaces[2];
                    if ($type == 'Vulcan' and $type2 == 'Scroll') {
                        $flag = true;
                        $str .= "§c> §eVulcan scroll detected...\n";
                        $str .= "§c> §eCombining scroll with tool...\n";
                        $user = $this->um->getOnlineUser($sender->getName());
                        if ($user->removeMoney(Data::$commandVulcanCost)) {
