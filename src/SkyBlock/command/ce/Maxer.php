...
class Maxer extends BaseCommand {
    /**
     * Maxer constructor.
     *
     * @param Main $plugin
    ...
    public function __construct(Main $plugin) {
        parent::__construct($plugin, 'maxer', 'Merge GOD scroll with the tool', '<ce name>');
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) : void {
        ...
        if ($this->func->countEnchants($hand, "ce") <= 0) {
            $this->sendMessage($sender, "§4[Error] §cThe item you're holding doesn't have any custom enchantments on it to max up!");
            return;
        }
        if ($hand->getTypeId() === 0) {
        ...
        if (($id = $this->func->getEnchantmentId($cename)) !== null) {
            ...
            if (($celevel = BaseEnchantment::getEnchantmentLevel($hand, $id)) < 6) {
                $this->sendMessage($sender, "§4[Error] §cThat CE specified doesn't have level 6 or 6+ to max out more! CE needs to have level 6 or more to use this scroll on it! Use levelup scroll for that!");
                return;
            }
            if ($celevel >= 10) {
                $this->sendMessage($sender, "§4[Error] §cThat CE specified is already maxed out to level 10!");
                return;
            }
        } else {
        ...
        foreach ($items as $slot => $item) {
            if ($item->getTypeId() === ItemTypeIds::PRISMARINE_SHARD) {
                if ($item->hasCustomName()) {
                    ...
                    $type2 = $spaces[2];
                    $level = mt_rand(1, 4);
                    if ($type == 'GOD' and $type2 == 'Scroll') {
                        $flag = true;
                        $str .= "§c> §e$cename of level $celevel detected on held item ...\n";
                        $str .= "§c> §eGOD scroll detected...\n";
                        ...
                        if ($user->removeMoney(Data::$commandMaxerCost)) {
                            ...
                            $str .= "§c> §eScroll merged with $cename for 10,000$!\n";
                            break;
                        } else {
                            $str .= "§c> §eCouldn't maxup the ce, you don't have 10,000$...\n";
                            break;
                        }
                    }
                }
